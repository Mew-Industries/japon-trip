...
- **UI**: los nodos "de paso" muestran "· de paso (por el día)". La lista "Qué hacer" está **colapsada por default** (toggle por card). Cada actividad geolocalizada linkea a su punto en el mapa; el link a Google Maps vive en el popup del punto. **Coords reales de Google** (no Nominatim) — ver `projects/japan-trip/data/master_coords.json`.
